...
public class ClassroomService {
  ...
  public ClassroomService(
      CustomOAuth2Util oAuth2Utils, @Qualifier("classroomWebClient") WebClient client) {
   ...
   */
  public <T extends ClassroomResponseType> Mono<T> getResponseMono(
      String path, ServerRequest request, Class<T> clazz) {
    return oAuth2Utils
        .extractAccessToken(request)
        .flatMap(token -> getResponseMono(path, token, clazz));
  }
   ...
   */
  public <T extends ClassroomResponseType> Mono<T> getResponseMono(
      String path, String token, Class<T> clazz) {
