...
public class CourseProgressHandler {
    ...
    public Mono<ServerResponse> getCourseProgressById(ServerRequest request) {
        return this.classroomService.getResponseMono("/courses/"+request.pathVariable("courseId"), request, ClassroomCourseResponseSingular.class)
                .flatMap(course -> this.calculateCourseProgress(course, request))
                .flatMap(courseProgress -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
    ...
    public Mono<ServerResponse> getCourseProgressForUser(ServerRequest request) {
        return this.classroomService.getResponseMono("/courses", request, ClassroomCourseResponsePlural.class)
                ...
                .flatMap(course -> this.calculateCourseProgress(course, request))
                .collectList()
                .flatMap(courseProgresses -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(courseProgresses))
                .switchIfEmpty(ServerResponse.notFound().build());
    }

    private Mono<CourseProgress> calculateCourseProgress(ClassroomCourseResponseSingular course, ServerRequest request) {
        return this.classroomService.getResponseMono("/courses/" + course.getId() + "/courseWork", request, ClassroomCourseWorkResponsePlural.class)
                .mapNotNull(ClassroomCourseWorkResponsePlural::getCourseWork)
                .flatMapMany(Flux::fromIterable)
                .flatMap(courseWork -> this.courseWorkIsHandedIn(course.getId(), courseWork.getId(), request))
                .reduce(new CourseProgress.TotalCompletedPair(), (subTotal, element) -> {
                    subTotal.incrementTotal();
                    if(element) {
                        subTotal.incrementCompleted();
                    }
                    return subTotal;
                })
                .switchIfEmpty(Mono.empty())
                .map(totalCompletedPair -> new CourseProgress(course.getId(), course.getName(), totalCompletedPair));
    }

    private Flux<Boolean> courseWorkIsHandedIn(String courseId, String courseWorkId, ServerRequest request) {
        return this.classroomService.getResponseMono("/courses/"+courseId+"/courseWork/"+courseWorkId+"/studentSubmissions", request, ClassroomStudentSubmissionResponsePlural.class)
                .map(ClassroomStudentSubmissionResponsePlural::getStudentSubmissions)
                .flatMapMany(Flux::fromIterable)
                .map(studentSubmission -> studentSubmission.getState().toUpperCase(Locale.ROOT).equals("TURNED_IN"));
    }
}
