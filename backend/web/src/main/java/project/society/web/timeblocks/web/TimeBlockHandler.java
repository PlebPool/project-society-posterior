...
public class TimeBlockHandler {
  ...
  public Mono<ServerResponse> initDb(ServerRequest request) {
    ...
    return this.oAuth2Util
        .extractPrincipalName(request)
        .flatMap(
            userId ->
                ServerResponse.ok()
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(
                        this.dayDAOService.save(new TimeBlockDay(userId, DayOfWeek.FRIDAY, list)),
                        TimeBlockDay.class));
  }

  /**
   * Gets target {@link TimeBlockDay} and adds request body as a {@link TimeBlock} to its array.
   * Returns the {@link TimeBlockDay}.
   *
  ...
  /**
   * Post endpoint for {@link TimeBlockDay}. Returns http 400 via exception if dayIndex is > 7 or <
   * 0. Returns posted {@link TimeBlockDay}.
   *
  ...
  public Mono<ServerResponse> postTimeBlockDay(ServerRequest request) {
    return request
        ...
        .zipWith(
            this.oAuth2Util.extractPrincipalName(request),
            ((timeBlockDay, principalName) -> {
  ...
  public Mono<ServerResponse> getTimeBlockById(ServerRequest request) {
    return this.dayDAOService
        .findOneById(request.pathVariable("uuid"))
