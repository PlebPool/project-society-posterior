...
public class GenericReactiveDAO {
   ...
   *
   * @param id Target id.
   * @param clazz Class containing {@link Id}, and {@link
   ...
   * @param clazz Class containing {@link Id}, and {@link
   *     org.springframework.data.relational.core.mapping.Table}.
   * @param <T> Type containing {@link Id}, and {@link
   *     org.springframework.data.relational.core.mapping.Table}.
  ...
  public <T extends HasId<?>> Mono<T> findOneById(String id, Class<T> clazz) {
    ...
    return this.r2dbcEntityTemplate.selectOne(
        Query.query(Criteria.where(idColumnName).is(id)), clazz);
  }
  ...
  public <T extends HasId<?>> Flux<T> idLike(String idToMatch, Class<T> clazz) {
    ...
    return this.r2dbcEntityTemplate.select(
        Query.query(Criteria.where(idColumnName).like(idToMatch)), clazz);
  }

  /**
   * Goes through the classes variable annotations, if a variable has the {@link Id} annotation. We
   * check if it also has a {@link Column} annotation, we then return the value from the {@link
   * Column} annotation. If it does not have a {@link Column} annotation, we return the field name
   * instead.
   *
