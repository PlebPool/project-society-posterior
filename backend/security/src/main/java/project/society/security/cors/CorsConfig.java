...
public class CorsConfig {
  ...
  public CorsConfigurationSource corsConfigurationSource(Environment environment) {
    ...
    boolean allowAllOrigins =
        Boolean.parseBoolean(environment.getProperty(PropertyNameHolder.PROJECT_DEV_CORS));
    CorsConfiguration cors = new CorsConfiguration();
    if (allowAllOrigins) {
      cors.addAllowedOriginPattern(CorsConfiguration.ALL);
    } else {
      Assert.hasLength(
          frontendOrigin, "Frontend origin cannot be empty if allow all origins is false.");
      cors.addAllowedOrigin(frontendOrigin);
    }
    cors.addAllowedHeader(
        String.valueOf(
            HttpHeaderNames.CONTENT_TYPE)); // TODO: MAKE REQUIRED FOR MUTATIVE HTTP METHODS.
